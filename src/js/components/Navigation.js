...
class Navigation extends React.Component {
  ...
  render() {
    const currentPath = this.props.router.location.pathname.split('/').join('')

    const classes = ['Navigation']
    if(this.props.relative === true) {
      classes.push('Navigation-relative')
    }
      ...
      <div className={classes.join(' ')}>
        ...
        <ul>
          {Content.routes.map((route, i) => {
            return <li key={route.path} className={currentPath === route.path.replace(/^\//, '') ? 'active' : ''}><Link to={route.path}>{i !== 0 ? i+'.' : null} {route.title}</Link></li>
          })}
          <li className="map" key="mapButton">
